...
//
// Brought into the tracked supabase/functions/ layout — same situation as
// whatsapp-webhook: this function's logic (ministry resolution, contact
// upsert, message save) was already complete, but it lived at
// supabase/evangelism-inbox-webhook/ instead of supabase/functions/, so it
// was never actually deployed. Messenger, Instagram, and Twilio-relayed
// WhatsApp inbound messages had nowhere to land. Registered in
// supabase/config.toml with verify_jwt = false, same as whatsapp-webhook —
// Meta/Twilio sign these requests themselves, not with a Supabase JWT.
//
