...
serve(async (req) => {
  ...
  try {
    ...
    if (!UNSUBSCRIBE_SECRET || UNSUBSCRIBE_SECRET === 'changeme') {
      console.error('[FATAL] Missing or default UNSUBSCRIBE_SECRET environment secret');
      throw new Error('UNSUBSCRIBE_SECRET is not configured or insecure. Cannot process broadcast emails.');
    }
