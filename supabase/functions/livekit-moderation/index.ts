  ...
  try {
    ...
    switch (body.action) {
      ...
      case 'remove-participant': {
        if (!body.identity) return json({ error: 'identity required' }, 400);
        // Real bug found live (2026-08-18): swallowing this error meant the
        // function ALWAYS reported success even when removeParticipant
        // genuinely failed — the caller had no way to ever know a removal
        // didn't take, and the UI's "Participant Removed" toast fired
        // regardless. "Participant not found" (already gone) is the one
        // case worth treating as a no-op success; every other failure now
        // actually surfaces.
        try {
          await svc.removeParticipant(body.roomName, body.identity);
        } catch (err) {
          const message = err instanceof Error ? err.message : String(err);
          if (!/not found/i.test(message)) return json({ error: `removeParticipant failed: ${message}` }, 502);
        }
        return json({ success: true });
      }
